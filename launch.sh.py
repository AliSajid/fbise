...
command_string = "docker run --detach --mount source=2018data,target=/app/fbise/data,type=volume --mount source=2018logs,target=/app/logs,type=volume --name worker{:0>2} localhost:5000/fbise /opt/conda/bin/python /app/fbise/downloader.py -s {} -e {} -l {} -p {} -t {}"
